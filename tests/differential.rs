...
use differential::writes::{
    ...
    run_poll_lifecycle_case, run_quote_lifecycle_case, run_relationship_writes_case,
    run_report_writes_case, run_status_creation_writes_case, run_status_interaction_writes_case,
...
#[tokio::test]
#[ignore = "requires guarded Mastodon/PostgreSQL/media clones from tools/mastodon-fixture"]
async fn quote_lifecycle() -> Result<(), Box<dyn std::error::Error>> {
    let repository_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let config = DifferentialConfig::from_process_environment(&repository_root)?;
    let writer_url = config
        .rust_write_database
        .as_ref()
        .expect("quote lifecycle requires a Rust writer URL")
        .url()
        .to_owned();
    let repository = Repository::connect(config.rust_database.url()).await?;
    ...
    let result = run_quote_lifecycle_case(config, &rust_url).await;
    let _ = shutdown_tx.send(());
    server.await??;
    result
}
