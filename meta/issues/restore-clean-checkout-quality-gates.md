...
- RED in a task-owned workspace on an isolated worker with no upstream source link: `cargo test --locked --all-targets --all-features --no-run` failed at the three compile-time includes. After replacing those, the full suite failed five Paperclip tests with missing fixture errors.
- GREEN on the same isolated-worker workspace without the source checkout: `cargo fmt --all --check`, `cargo test --locked --all-targets --all-features` (406 passed, 125 explicitly ignored across 22 binaries), and `cargo clippy --locked --all-targets --all-features -- -D warnings` passed. Remote `cmp` verified all three media files against the pinned Git blobs.
