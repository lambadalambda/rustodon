...
- In progress: phase 2 implementation prepared after the parent recorded the phase-1 isolated worker red baseline; awaiting parent-run green verification.
...
- The supported browser automation tooling must be initialized before use.
- Read-only pinned source came from a historical external reference artifact not
  retained in the repository, including
  `app/javascript/mastodon/actions/settings.js` and
  `spec/requests/api/web/settings_spec.rb` from cached image `696439...0cf`. The
  source checkouts were unavailable; no source was fetched.
...
- Parent-reported isolated-worker red baseline: **95 failures** from missing action/save/API gates; the artifact is not in the repository. This evidence was reported by the parent, not executed or inspected here.
...
- Parent next: sync only the three scoped files and run the isolated browser
  cutover through the documented isolated fixture. CLI/browser compatibility and
  real action behavior still require runtime evidence before claiming browser green.
- No local test/build/fmt/browser workloads, external-worker execution, or commits performed here. Issue remains open pending real-browser green evidence.

## Verified CLI compatibility follow-up

- Parent reports **shell + Node JS checks green** on an isolated worker with Node
  24.15.0 and the supported browser tooling.
- Real cutover stopped immediately at the pre-existing unsupported `--pin-tab`
  flag. Removing only that flag retained the verified session and argument
  behavior; run-specific sessions preserved isolation between concurrent
  invocations.
- Regression mocks reject the old invocation, require one consistent run-specific
  session per operation, verify exact-session cleanup, and distinguish concurrent
  smoke invocations. API observation and frontend action logic are unchanged.
- Independent read-only review found no blocking CLI-isolation or test findings. Parent subsequently reports the CLI-adjusted shell/JS checks green. No commands, tests, browser/external-worker workloads, or commits executed here during that follow-up.

## Bounded progress diagnostics follow-up

- Parent reports real cutover exits **124** after schema readiness with no browser progress in a historical external run. This does not establish a Chromium/capability/sandbox cause. Previously session discovery, initial `network requests --clear` (which may initialize the browser), `open`, and cleanup had no wrapper deadline or progress output; only waits/evals were bounded.
...
- Scope remains the two owned scripts and this issue. Tests-first additions are prepared but red/green execution is explicitly deferred: parent owns all tests/workloads, external-worker, and commits.
...
remaining browser blocker—not unexecuted browser infrastructure or a startup 404.

## Actual browser acceptance (isolated worker)

The HTTP diagnostic conclusively identified CSRF 422 despite a valid payload and
15 ms completion. The separate HTTPS-fixture issue fixes transport, not production
cookie policy or debounce timing. A historical external run records **browser
...
are recorded separately. Offline extracted-JS/shell and TLS tests pass on an isolated worker;
independent merged integration review approved with no blocking findings.

Final acceptance: a historical external run passed browser actions **and** full
cutover rollback after exact fixture-user settings snapshot/restoration was added.
An intermediate run observed a still-pending PUT at the unchanged leading-response
deadline; it is retained as a failed run, not relabeled green. The successful run
used identical browser predicates, TLS policy, and timing bounds.
